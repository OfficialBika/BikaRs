...
function getBroadcastConcurrency() {
  const value = Number(process.env.BROADCAST_CONCURRENCY || 20);
...
async function runBroadcastJob(telegram, jobId) {
  ...
  try {
    ...
    const concurrency = getBroadcastConcurrency();
    let batch = [];
    ...
    for await (const user of cursor) {
      ...
      if (batch.length >= concurrency) {
        ...
        job.updatedAt = new Date();
        await job.save();
        processedSinceEdit += concurrency;
